...
specific to the macOS Apple Silicon host described below.
...
is the rom-converto-only output of the harness. The external tool is
compress only, so the decompress rows have no external column.
