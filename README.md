...
* [x] Decompress `.rvz` back to raw `.iso`
* [x] Migrate legacy `.wia` images (all codecs: bzip2, LZMA, LZMA2, purge, none) and `.nkit.iso` / `.nkit.gcz` images to `.rvz` with integrity checks first, rebuilding the hash tree and re-encrypting partitions on the fly
...
| `-f, --force` | `compress`, `migrate`, `decompress` | Alias for `--on-conflict overwrite` (on `migrate`, overwrite the output if it already exists) |
| `-R, --recursive` | `compress`, `migrate`, `decompress`, `verify` | Recursively process every matching file in INPUT and its subdirectories |
