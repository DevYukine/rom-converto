//! Warm-run statistics and the string formatting used in benchmark result
//! tables.
...
/// Format a duration in seconds with millisecond precision, for example
/// `3.142 s`.
pub fn fmt_seconds(seconds: f64) -> String {
    format!("{seconds:.3} s")
}

/// rom-converto / reference ratio, with a parenthesized speed-up when
...
/// Signed byte delta with thousands separators plus a percentage, for
/// example `-63,268 B (-0.0029 %)`, matching the `.md` size-delta column.
...
/// Humanized byte size for the Output column, for example `915 MB`, `1.4 GB`.
