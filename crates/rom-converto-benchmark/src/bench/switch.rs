//! Switch NSP and XCI compression, decompression, and verification
//! benchmarked against nsz, plus a cross-tool round-trip check.
...
/// Benchmark Switch NSP and/or XCI compression, decompression, and
/// verification against nsz for each configured input.
pub fn run(
    ctx: &BenchCtx,
    nsp: Option<PathBuf>,
    xci: Option<PathBuf>,
    ...
    if table.rows.is_empty() {
        bail!(
            "no Switch inputs configured; pass --nsp and/or --xci, or set \
             ROMCONVERTO_BENCH_NX_NSP / ROMCONVERTO_BENCH_NX_XCI"
        );
    }
    table.print();
    Ok(())
...
fn bench_container(
    ...
    // Verify is rom-converto only; nsz has no verify equivalent.
