//! 3DS ROM and CIA compression benchmarked against z3ds_compressor, plus a
//! rom-converto-only decompress pass and a cross-tool round-trip check.
...
/// Benchmark 3DS ROM and/or CIA compression against z3ds_compressor for
/// each configured input.
pub fn run(ctx: &BenchCtx, three_ds: Option<PathBuf>, cia: Option<PathBuf>) -> Result<()> {
    ...
    if table.rows.is_empty() {
        bail!(
            "no 3DS inputs configured; pass --three-ds and/or --cia, or set \
             ROMCONVERTO_BENCH_CTR_3DS / ROMCONVERTO_BENCH_CTR_CIA; inputs must \
             already be decrypted with rom-converto ctr decrypt"
        );
    }
    table.print();
    Ok(())
...
fn cross_check(
    ...
    println!(
        ...
        if ok { "OK (matches source)" } else { "FAIL" }
    );
    Ok(())
}
