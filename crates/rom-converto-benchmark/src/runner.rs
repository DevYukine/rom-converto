//! Timed run loops that interleave a reference tool and rom-converto,
//! discarding the first (cold) sample on each side.
...
/// Per-run knobs shared across every platform benchmark.
#[derive(Clone)]
...
/// Kills leftover tool processes between runs on a best-effort basis,
/// mirroring the scripts' `pkill`/`taskkill`. Exact-name matching keeps the
/// benchmark process (rom-converto-benchmark) from being caught by
/// `rom-converto`.
