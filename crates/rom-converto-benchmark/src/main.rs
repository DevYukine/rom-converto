...
/// Reproducible benchmarks comparing rom-converto against the same external
/// reference tools and methodology documented in benchmark/*.md.
#[derive(Parser)]
#[command(name = "rom-converto-benchmark", version, about, long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
fn run_all(ctx: &BenchCtx) -> Result<()> {
    ...
    if attempted == 0 {
        bail!(
            "`all` ran nothing: set the ROMCONVERTO_BENCH_* variables for the platforms you want."
        );
    }
    Ok(())
}
