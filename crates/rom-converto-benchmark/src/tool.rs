//! Resolution of the rom-converto binary under test and lookup of reference
//! tools on PATH or beside it.
...
/// directory holding the resolved rom-converto binary. Missing tools error
/// with install and placement guidance.
pub fn find_tool(base: &str, rom_converto_dir: &Path) -> Result<PathBuf> {
    ...
    Err(anyhow!(
        "required tool {base} was not found; install it, add it to PATH, or place \
         the executable in the rom-converto directory ({})",
