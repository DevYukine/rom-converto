...
pub async fn ps3_decrypt(run: &BatchRun<'_>, skip_probe: bool) -> Result<()> {
    let BatchRun {
        progress,
        total_progress,
        input_dir,
    ...
    for path in files {
        ...
        .await
        {
            match e {
                ...
                _ => {
                    warn!("Failed to decrypt {}: {e}", path.display());
                    tally.record_failed();
                    records.push(failed_record(&path, "decrypt", input_bytes, started, e));
                }
            }
        } else {
            let out_bytes = file_len(&out_path);
