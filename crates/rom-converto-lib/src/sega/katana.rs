...
pub fn parse(head: &[u8]) -> Result<KatanaInfo> {
    let found = probe_sega_disc(head)
        .filter(|h| h.system == SegaDiscSystem::Dreamcast)
        .ok_or_else(|| anyhow!("katana: first sector carries no \"SEGA SEGAKATANA \" id"))?;
    let ip = head
        .get(found.offset..found.offset + 0x100)
        .ok_or_else(|| anyhow!("katana: first sector is shorter than the IP header"))?;
...
pub fn parse_gdi(path: &Path) -> Result<KatanaInfo> {
    ...
    let third = index
        .tracks
        .get(2)
        .ok_or_else(|| anyhow!("katana: gdi index lists fewer than three tracks"))?;
    let track_path = path
        .parent()
        .unwrap_or(Path::new("."))
        .join(&third.filename);
...
fn parse_track(line: &str) -> Result<GdiTrack> {
    let bad = || anyhow!("katana: malformed gdi track line {line:?}");
