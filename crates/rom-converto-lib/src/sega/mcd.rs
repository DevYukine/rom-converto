...
use super::{SegaDiscSystem, md, probe_sega_disc};
...
pub fn parse(head: &[u8]) -> Result<McdInfo> {
    let found = probe_sega_disc(head)
        .filter(|h| h.system == SegaDiscSystem::SegaCd)
        .ok_or_else(|| anyhow!("mcd: first sector carries no \"SEGADISCSYSTEM\" id"))?;
    let boot = head
        .get(found.offset..found.offset + 0x200)
        .ok_or_else(|| anyhow!("mcd: first sector is shorter than the boot header"))?;
