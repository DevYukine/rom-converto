...
pub fn parse(data: &[u8]) -> Result<HandyInfo> {
    let header = data
        .get(..HEADER_LEN)
        .ok_or_else(|| anyhow!("handy: file shorter than the 64-byte LNX header"))?;
    if header[..4] != MAGIC {
        return Err(anyhow!("handy: bad magic, expected \"LYNX\""));
    }
