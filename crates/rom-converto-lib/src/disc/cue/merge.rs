...
mod tests {
    use super::*;
    use crate::disc::cue::models::{CueFile, FileType, Index, Track, TrackType};
    ...
    async fn non_binary_file_is_rejected() {
        ...
        let err = merge_bin(&NoProgress, cue_path, out_cue, false, CancelToken::new())
            .await
            .unwrap_err();
    ...
    async fn output_colliding_with_input_is_refused_even_with_force() {
        ...
        let err = merge_bin(&NoProgress, cue_path, out_cue, true, CancelToken::new())
            .await
            .unwrap_err();
    ...
    async fn cancelled_merge_preserves_existing_outputs() {
        ...
        let err = merge_bin(&NoProgress, cue_path, output_cue.clone(), true, cancel)
            .await
            .unwrap_err();
