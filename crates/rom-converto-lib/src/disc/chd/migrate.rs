...
use crate::disc::chd::legacy::LEGACY_COMPRESSION_AV;
...
mod tests {
    use super::*;
    use crate::disc::cd::CD_HUNK_BYTES;
    ...
    async fn migrate_image(image: &[u8]) -> crate::disc::chd::reader::SyncChdHandle {
        ...
        verify_chd(&NoProgress, out.clone(), None, false, CancelToken::new())
            .await
            .unwrap();
        crate::disc::chd::reader::open_chd_sync(&out).unwrap()
    }
    ...
    async fn migrate_v1_chd_carries_geometry_into_gddd() {
        ...
        let handle = crate::disc::chd::reader::open_chd_sync(&out).unwrap();
