...
/// Hard disk geometry: `CYLS:%d,HEADS:%d,SECS:%d,BPS:%d`.
pub const CHD_METADATA_TAG_HARD_DISK: [u8; 4] = *b"GDDD";
...
/// Byte offsets of the two SHA-1 fields from the start of the file,
/// summed over the field layout above so the in-place hash fix in
/// `verify` cannot drift from the struct.
pub const CHD_HEADER_RAW_SHA1_OFFSET: u64 = 8 + 4 + 4 + 4 * 4 + 3 * 8 + 4 + 4;
pub const CHD_HEADER_SHA1_OFFSET: u64 = CHD_HEADER_RAW_SHA1_OFFSET + SHA1_BYTES as u64;
const _: () = assert!(
    CHD_HEADER_SHA1_OFFSET + 2 * SHA1_BYTES as u64 == CHD_V5_HEADER_SIZE as u64,
    "SHA-1 offsets must cover the header through its trailing parent_sha1"
);
