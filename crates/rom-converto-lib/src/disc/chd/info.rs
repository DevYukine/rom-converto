...
use crate::disc::chd::reader::cue_generator::parse_chd_track_metadata;
use crate::disc::chd::reader::{chd_version, open_chd_sync};
pub use crate::disc::laserdisc::vbi::{LdClvTime, LdDiscType};
...
fn add_metadata(mut info: ChdInfo, metadata: &[ChdMetadataHeader], path: &Path) -> Result<ChdInfo> {
    ...
    info.content = crate::sony::chd_disc_content(path);
    Ok(info)
}
...
fn extract_hard_disk_info(metadata: &[ChdMetadataHeader]) -> Option<ChdHardDiskInfo> {
    let entry = metadata
        .iter()
        .find(|m| m.tag == crate::disc::chd::models::CHD_METADATA_TAG_HARD_DISK)?;
    let (cylinders, heads, sectors, sector_bytes) =
        crate::disc::chd::legacy::parse_gddd(&entry.data)?;
    ...
    let avld = metadata
        ...
        .filter(|data| {
            !data.is_empty()
                && data
                    .len()
                    .is_multiple_of(crate::disc::laserdisc::vbi::VBI_PACKED_BYTES)
        });

    let (frame_count, vbi) = match avld {
        Some(data) => (
            (data.len() / crate::disc::laserdisc::vbi::VBI_PACKED_BYTES) as u32,
...
/// [`crate::disc::laserdisc::vbi::vbi_metadata_pack`].
fn summarize_vbi(data: &[u8]) -> ChdLdVbiInfo {
    use crate::disc::laserdisc::vbi::{
...
fn extract_tracks(metadata: &[ChdMetadataHeader]) -> Vec<ChdTrack> {
    let Some(meta_str) = crate::disc::chd::cd_track_metadata_text(metadata) else {
        return Vec::new();
    };
...
fn into_anyhow(e: crate::disc::chd::error::ChdError) -> anyhow::Error {
    anyhow::anyhow!("chd: {}", e)
}
...
mod tests {
    use super::*;
    use crate::disc::chd::legacy::tests::{Fixture, TestHunk};
    use crate::disc::chd::models::CHD_METADATA_RESERVED_BYTES;
    use crate::disc::laserdisc::vbi::{
        VBI_CODE_LEADIN, VBI_CODE_LEADOUT, VBI_PACKED_BYTES, VbiMetadata,
    };
    use std::io::Write;
