...
/// Computes a pair of ECC values for one row of the P or Q matrix.
#[inline]
fn ecc_compute_bytes(sector: &[u8], row: &[u16]) -> (u8, u8) {
    let mut val1: u8 = 0;
    let mut val2: u8 = 0;
    for &offset in row {
        val1 ^= ecc_source_byte(sector, offset as usize);
        val2 ^= ecc_source_byte(sector, offset as usize);
        val1 = ECCLOW[val1 as usize];
    }
    val1 = ECCHIGH[(ECCLOW[val1 as usize] ^ val2) as usize];
    val2 ^= val1;
    (val1, val2)
}

/// Verifies that the ECC P and Q bytes in a sector are correct.
pub fn ecc_verify(sector: &[u8]) -> bool {
    ...
    for byte in 0..ECC_Q_NUM_BYTES {
        ...
        if sector[ECC_Q_OFFSET + byte] != val1
            || sector[ECC_Q_OFFSET + ECC_Q_NUM_BYTES + byte] != val2
        {
            return false;
        }
    }
...
/// Zeros out the ECC P and Q parity bytes in a sector.
pub fn ecc_clear(sector: &mut [u8]) {
    debug_assert!(sector.len() >= SECTOR_SIZE);
    sector[ECC_P_OFFSET..ECC_P_OFFSET + 2 * ECC_P_NUM_BYTES].fill(0);
    sector[ECC_Q_OFFSET..ECC_Q_OFFSET + 2 * ECC_Q_NUM_BYTES].fill(0);
}

/// Regenerates the ECC P and Q parity bytes from the sector data.
...
/// Checks if a sector has a valid sync header and correct ECC.
pub fn has_valid_ecc(sector: &[u8]) -> bool {
...
/// Strips ECC data from a Mode 1 sector for better compression.
/// Zeros the 12-byte sync header and the ECC P/Q parity bytes.
...
/// Restores ECC data in a Mode 1 sector after decompression.
