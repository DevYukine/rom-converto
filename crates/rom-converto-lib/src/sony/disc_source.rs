...
use crate::disc::chd::legacy::LegacyChd;
use crate::disc::chd::models::{CHD_METADATA_TAG_DVD, ChdMetadataHeader};
use crate::disc::chd::padded_track_frames;
use crate::disc::chd::reader::cue_generator::{ChdTrackInfo, parse_chd_track_metadata};
...
use crate::disc::chd::reader::{SyncChdHandle, chd_version, open_chd_sync};
use crate::disc::cue::CueParser;
use crate::disc::cue::models::{FileType, TrackType};
use crate::disc::cue::to_iso::extract_user_data;
...
impl CueSectors {
    ...
    pub fn open(path: &Path) -> io::Result<Self> {
        ...
        Ok(Self {
            ...
            start_lba,
            sectors,
            raw: vec![0u8; track.track_type.block_size() as usize],
        })
    }
}
...
impl SectorSource for CueSectors {
    fn read_sector(&mut self, lba: u32, buf: &mut [u8; SECTOR]) -> io::Result<()> {
        ...
        let block = self.track_type.block_size() as u64;
        file_read_exact_at(
            &self.bin,
            &mut self.raw,
            (self.start_lba + lba as u64) * block,
        )?;
        buf.copy_from_slice(
            extract_user_data(self.track_type, &self.raw, lba as u64).map_err(io::Error::other)?,
        );
        Ok(())
    }
...
fn first_data_track(metadata: &[ChdMetadataHeader]) -> io::Result<(TrackType, u64, u64)> {
    let text = crate::disc::chd::cd_track_metadata_text(metadata)
