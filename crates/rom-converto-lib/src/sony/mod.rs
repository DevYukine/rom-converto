...
//! `PSP_GAME/PARAM.SFO`. The probe runs over
//! [`crate::util::iso9660::SectorSource`], so the same reader serves a
//! plain `.iso`, a `.cue`/`.bin` pair, and the discs carried inside CSO
...
pub fn read_disc_content<S: SectorSource>(src: &mut S) -> io::Result<Option<DiscContent>> {
    ...
    Ok(match volume.kind {
        DiscKind::Psp => Some(DiscContent::Psp(psp::disc::read(src, &volume)?)),
...
pub fn read_psx_info(path: &Path) -> Result<PsxInfo> {
    if is_cue(path) {
        let mut src = disc_source::CueSectors::open(path)?;
...
pub fn read_psp_info(path: &Path) -> Result<PspInfo> {
    ...
    Ok(psp::disc::read(&mut src, &volume)?)
}
...
pub(crate) fn chd_disc_content(path: &Path) -> Option<DiscContent> {
    let mut src = disc_source::ChdSectors::open(path).ok()?;
...
pub(crate) fn cso_disc_content(path: &Path) -> Option<DiscContent> {
    let mut src = disc_source::CsoSectors::open(path).ok()?;
    read_disc_content(&mut src).ok().flatten()
}
...
mod tests {
    ...
    use super::*;
    use crate::disc::cue::models::TrackType;
    ...
    async fn reads_ps2_dvd_and_ps1_cd_inside_a_chd() {
        use crate::disc::chd::{ChdOptions, convert_disc_to_chd};
        use crate::util::NoProgress;
        ...
        for (name, iso, want) in [
            ...
            let info = crate::disc::chd::info::read_info(&chd).expect("read chd info");
