...
use crate::nintendo::disc::input::{disc_size_of, open_disc_input};
use crate::nintendo::disc::rvz::constants::{
...
use crate::nintendo::disc::rvz::regions::{DiscRegion, RegionPlan};
...
/// [`crate::nintendo::disc::legacy::migrate_disc`], which
...
pub async fn compress_disc(
    ...
    let legacy = {
        ...
        task::spawn_blocking(move || crate::nintendo::disc::legacy::detect_legacy_format(&input))
            .await??
    };
    if legacy.is_some() {
        return crate::nintendo::disc::legacy::migrate_disc(
            input,
            output,
            options,
            crate::nintendo::disc::legacy::MigrateOptions::default(),
            crate::nintendo::disc::legacy::ALL_MIGRATE_FORMATS,
            progress,
            cancel,
        )
        .await;
...
fn write_tables_and_head(
    ...
    let part_off = if !tables.partitions.is_empty() {
        ...
        for part in &tables.partitions {
            let mut bytes = Vec::with_capacity(crate::nintendo::disc::rvz::format::WIA_PART_SIZE);
    ...
    let disc = WiaDisc {
        ...
        part_t_size: crate::nintendo::disc::rvz::format::WIA_PART_SIZE as u32,
        part_off,
        part_hash,
