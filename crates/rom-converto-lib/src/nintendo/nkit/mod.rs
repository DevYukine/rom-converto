...
#[cfg(test)]
mod wii_tests {
    use super::test_fixtures::{make_fake_wii_fs_iso, make_fake_wii_fs_iso_partial, make_nkit_wii};
    use super::*;
    ...
    // The partition ends on a partial final group, so its data size is
    // not 0x8000-aligned. NKit's junk record spans the trailing partial
    // block that Nintendo zero-padded, so a byte-identical restore
    // depends on zero_pad_partial_tail forcing that tail back to zero.
    #[test]
    fn nkit_wii_partial_final_group_restores_byte_identical() {
        let iso = make_fake_wii_fs_iso_partial();
        let nkit = make_nkit_wii(&iso);
        assert!(nkit.len() < iso.len(), "stripping must shrink the image");
        ...
        assert!(
            out == iso,
            "partial-tail Wii restoration must be byte-identical"
        );
    }
