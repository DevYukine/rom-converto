...
use crate::nintendo::rvl::constants::{
    ...
    WII_SECTOR_SIZE, WII_SECTOR_SIZE_U64, WII_TICKET_TITLE_ID_OFFSET, WII_TICKET_TITLE_KEY_OFFSET,
};
...
/// Number of data sectors in a partition whose final group is partial,
/// so the restored data size is not 0x8000-aligned. 168 = two whole
/// groups plus 40 sectors; 168 is not a multiple of 32, so 168 * 0x7C00
/// leaves a 0x6000 tail past the last full 0x8000 block.
const PARTIAL_TAIL_SECTORS: usize = 2 * WII_BLOCKS_PER_GROUP + 40;

fn group_blocks_at(data_sectors: usize, group: usize) -> usize {
    (data_sectors - group * WII_BLOCKS_PER_GROUP).min(WII_BLOCKS_PER_GROUP)
}
...
/// Same fixture but with a partial final group, so the partition data
/// size is not 0x8000-aligned and NKit's junk gap spills into a tail
/// that Nintendo zero-padded. Exercises `zero_pad_partial_tail`.
pub(crate) fn make_fake_wii_fs_iso_partial() -> Vec<u8> {
    fake_wii_fs_iso(PARTIAL_TAIL_SECTORS)
}

fn fake_wii_fs_iso(data_sectors: usize) -> Vec<u8> {
    let data_size = data_sectors * WII_SECTOR_PAYLOAD_SIZE;
    let hashed_size = data_sectors as u64 * WII_SECTOR_SIZE_U64;
    ...
    cipher
        .encrypt_padded_b2b::<block_padding::NoPadding>(&plaintext_title_key, &mut enc_key)
        .unwrap();
    ...
    // Nintendo fills whole 0x8000 blocks with junk and zero-pads the
    // trailing partial block; the junk stream stops at the last full
    // block boundary and the rest of the partition data stays zero.
    let junk_from = 0x49000 + 0x1C;
    let junk_to = (data_size & !(WII_SECTOR_SIZE - 1)).max(junk_from);
    let tail = junk_at_id(WII_PART_ID, DISC_NUM, junk_from as u64, junk_to - junk_from);
    ...
    for g in 0..groups {
        ...
        for (s, p) in payloads.iter_mut().enumerate() {
            let off = g * WII_GROUP_PAYLOAD_SIZE as usize + s * WII_SECTOR_PAYLOAD_SIZE;
            if s < blocks {
        ...
        reencrypt_cluster_into(&regions, &payloads, &plaintext_title_key, &mut cluster_out)
            .unwrap();
        let off = data_start + written_sectors * WII_SECTOR_SIZE;
        iso[off..off + blocks * WII_SECTOR_SIZE]
            .copy_from_slice(&cluster_out[..blocks * WII_SECTOR_SIZE]);
        written_sectors += blocks;
    }
...
/// Emit the partition's trailing gap as a single pure-junk record.
/// Nintendo zero-pads the final partial 0x8000 block, but NKit's junk
/// record still spans it, so the record covers the zero tail and the
/// reader must force that tail back to zero on restore.
fn emit_trailing_junk_gap(out: &mut Vec<u8>, data: &[u8], from: usize, lead: usize) {
    let len = data.len() - from;
    assert_eq!(len % 4, 0, "gaps are 4-byte aligned");
    let junk_to = (data.len() & !(WII_SECTOR_SIZE - 1)).max(from + lead);
    assert!(data[from..from + lead].iter().all(|&b| b == 0));
    assert!(
        data[from + lead..junk_to]
            == junk_at_id(
                WII_PART_ID,
                DISC_NUM,
                (from + lead) as u64,
    ...
    assert!(data[junk_to..].iter().all(|&b| b == 0));
    out.extend_from_slice(&(len as u32).to_be_bytes());
}
...
pub(crate) fn make_nkit_wii(iso: &[u8]) -> Vec<u8> {
    let data_start = PART_OFFSET + PART_DATA_OFFSET;
    let ds_off = PART_OFFSET + WII_PARTITION_HEADER_DATA_SIZE_OFFSET;
    let hashed_size = u32::from_be_bytes(iso[ds_off..ds_off + 4].try_into().unwrap()) as u64 * 4;
    let data_sectors = (hashed_size / WII_SECTOR_SIZE_U64) as usize;
    let data_size = data_sectors * WII_SECTOR_PAYLOAD_SIZE;
    let groups = hashed_size.div_ceil(WII_GROUP_TOTAL_SIZE) as usize;
    let title_key = [0x5Au8; 16];

    // Decrypt the partition into hashless data plus on-disc hash
    // regions; the final group may carry fewer than a full 64 sectors.
    let mut data = vec![0u8; data_size];
    let mut on_disc = vec![[0u8; HASH_REGION_BYTES]; data_sectors];
    let mut read_sectors = 0usize;
    for g in 0..groups {
        let blocks = group_blocks_at(data_sectors, g);
        for s in 0..blocks {
            let off = data_start + (read_sectors + s) * WII_SECTOR_SIZE;
            ...
            on_disc[read_sectors + s].copy_from_slice(&sector[..HASH_REGION_BYTES]);
    ...
    // Partial groups hash as a full group with the missing sectors
    // zero-padded, mirroring the reader.
    let mut payloads = vec![[0u8; WII_SECTOR_PAYLOAD_SIZE]; WII_BLOCKS_PER_GROUP];
    let mut regions = vec![[0u8; HASH_REGION_BYTES]; WII_BLOCKS_PER_GROUP];
    ...
    for g in 0..groups {
        ...
        for (s, p) in payloads.iter_mut().enumerate() {
            let doff = g * WII_GROUP_PAYLOAD_SIZE as usize + s * WII_SECTOR_PAYLOAD_SIZE;
            if s < blocks {
                p.copy_from_slice(&data[doff..doff + WII_SECTOR_PAYLOAD_SIZE]);
            } else {
                *p = [0u8; WII_SECTOR_PAYLOAD_SIZE];
            }
        }
    ...
    if src_pos < data.len() {
        ...
        emit_trailing_junk_gap(&mut pdata, &data, src_pos, lead);
    }
