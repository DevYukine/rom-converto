...
pub fn parse(data: &[u8]) -> Result<AgbInfo> {
    if data.len() < HEADER_END {
        return Err(anyhow!("agb: file shorter than the 0xC0-byte header"));
    }
    if data[0xB2] != 0x96 {
        return Err(anyhow!("agb: fixed header byte at 0xB2 is not 0x96"));
    }
