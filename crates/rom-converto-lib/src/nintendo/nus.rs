...
pub fn parse(data: &[u8]) -> Result<NusInfo> {
    let head = data
        .get(..HEAD_LEN)
        .ok_or_else(|| anyhow!("nus: file shorter than the 4 KiB header and boot code"))?;
    ...
    let (byte_order, head) = match head[..4] {
        ...
        _ => return Err(anyhow!("nus: unrecognized byte order signature")),
    };
