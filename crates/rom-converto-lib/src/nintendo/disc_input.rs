...
use crate::nintendo::gcz::error::GczError;
use crate::nintendo::legacy_input::{LegacyFormat, detect_legacy_format};
use crate::nintendo::nkit::NkitReader;
use crate::nintendo::nkit::error::NkitError;
use crate::nintendo::rvz::decompress::RvzDiscReader;
use crate::nintendo::rvz::error::RvzError;
use crate::nintendo::rvz::format::{WIA_FILE_HEAD_SIZE, WiaFileHead};
use crate::nintendo::wbfs::WbfsReader;
use crate::nintendo::wbfs::error::WbfsError;
use crate::nintendo::wia::WiaReader;
use crate::nintendo::wia::error::WiaError;
...
pub fn disc_size_of(path: &Path) -> DiscInputResult<u64> {
    ...
    match detect_legacy_format(path).unwrap_or(None) {
        ...
        Some(LegacyFormat::NkitGcz) => {
            let dhead = crate::nintendo::gcz::gcz_logical_prefix(path, 0x440)?;
            Ok(crate::nintendo::nkit::format::NkitHeader::parse(&dhead)?.image_size)
        }
