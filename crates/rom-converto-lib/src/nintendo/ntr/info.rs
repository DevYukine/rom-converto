...
//! encryption state, and the decoded banner (titles + icon), for `ntr info`.

use crate::info::{Image, LanguageCode, MultilingualString};
use crate::nintendo::ntr::{
...
pub struct NtrInfo {
    ...
    pub secure_area: NtrSecureAreaState,
    pub banner: Option<NtrBannerInfo>,
}
...
pub fn read_info(path: &Path) -> Result<NtrInfo> {
    let physical_bytes = std::fs::metadata(path)
        .with_context(|| format!("ntr info: stat {}", path.display()))?
        .len();

    let mut file =
        File::open(path).with_context(|| format!("ntr info: open {}", path.display()))?;

    let mut header = [0u8; HEADER_SIZE];
    file.read_exact(&mut header)
        .context("ntr info: read header")?;
    ...
    let secure_area =
        detect_secure_area(&mut file, arm9.rom_offset).context("ntr info: read secure area")?;

    let banner = if banner_offset != 0 {
        read_banner(&mut file, banner_offset as u64).unwrap_or_else(|e| {
            log::debug!("ntr info: banner read skipped ({})", e);
            None
        })
    } else {
        None
...
fn read_arm_info(bytes: &[u8]) -> NtrArmInfo {
    NtrArmInfo {
...
fn detect_secure_area(file: &mut File, arm9_rom_offset: u32) -> Result<NtrSecureAreaState> {
    if !(SECURE_AREA_OFFSET..SECURE_AREA_END).contains(&(arm9_rom_offset as usize)) {
        return Ok(NtrSecureAreaState::NotPresent);
    }
    ...
    if head == *DECRYPTED_MARKER || head == *SECURE_AREA_ID {
        Ok(NtrSecureAreaState::Decrypted)
    } else {
        Ok(NtrSecureAreaState::Encrypted)
    }
}
...
fn read_banner<R: Read + Seek>(reader: &mut R, offset: u64) -> Result<Option<NtrBannerInfo>> {
...
mod tests {
    use super::*;
    use crate::nintendo::ntr::test_fixtures::{
        SYNTH_TITLE, make_ntr_info_fixture, make_ntr_info_fixture_no_banner,
    };
    ...
    fn reads_header_fields() {
        ...
        std::fs::write(&path, make_ntr_info_fixture(NtrSecureAreaState::Decrypted)).unwrap();
    ...
    fn header_crc_invalid_when_corrupted() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("game.nds");
        let mut rom = make_ntr_info_fixture(NtrSecureAreaState::Decrypted);
    ...
    fn no_banner_pointer_yields_no_banner() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("game.nds");
        std::fs::write(&path, make_ntr_info_fixture_no_banner()).unwrap();
    ...
    fn secure_area_not_present_when_arm9_offset_outside_window() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("game.nds");
        std::fs::write(&path, make_ntr_info_fixture(NtrSecureAreaState::NotPresent)).unwrap();

        let info = read_info(&path).unwrap();
        assert_eq!(info.secure_area, NtrSecureAreaState::NotPresent);
    }
    ...
    fn secure_area_detected_encrypted() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("game.nds");
        std::fs::write(&path, make_ntr_info_fixture(NtrSecureAreaState::Encrypted)).unwrap();

        let info = read_info(&path).unwrap();
        assert_eq!(info.secure_area, NtrSecureAreaState::Encrypted);
    }
    ...
    fn secure_area_detected_decrypted() {
        ...
        let info = read_info(&path).unwrap();
        assert_eq!(info.secure_area, NtrSecureAreaState::Decrypted);
    }
}
