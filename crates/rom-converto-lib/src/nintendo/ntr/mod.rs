...
pub use error::{NtrError, NtrResult};
pub use key1::Key1;
...
pub fn detect_state(
    ...
    Err(NtrError::SecureAreaCorrupt)
}
...
pub async fn encrypt_ntr_rom(
    ...
    crypt_ntr_rom(progress, input_path, output_path, force, cancel, true).await
}

/// Decrypts a ROM's secure area, copying the rest of the file unchanged.
pub async fn decrypt_ntr_rom(
    progress: &dyn ProgressReporter,
    input_path: PathBuf,
    output_path: PathBuf,
    force: bool,
    ...
    crypt_ntr_rom(progress, input_path, output_path, force, cancel, false).await
}

async fn crypt_ntr_rom(
    progress: &dyn ProgressReporter,
    input_path: PathBuf,
    output_path: PathBuf,
    force: bool,
    ...
    let secure = tokio::task::spawn_blocking(move || -> NtrResult<[u8; SECURE_BLOCK_LEN]> {
        ...
        match (detect_state(&header, &secure)?, encrypt) {
            (SecureAreaState::Encrypted, true) => return Err(NtrError::AlreadyEncrypted),
            (SecureAreaState::Decrypted, false) => return Err(NtrError::AlreadyDecrypted),
            _ => {}
        }
...
#[cfg(test)]
mod tests {
    use super::test_fixtures::{SYNTH_IDCODE, synth_ntr};
    use super::*;
    ...
    fn state(rom: &[u8]) -> NtrResult<SecureAreaState> {
    ...
    fn fresh_synth_reads_as_decrypted() {
        let rom = synth_ntr(SYNTH_IDCODE, SECURE_AREA_OFFSET as u32);
        assert_eq!(state(&rom).expect("detects"), SecureAreaState::Decrypted);
    }

    #[test]
    fn plain_encryobj_head_reads_as_decrypted() {
        let mut rom = synth_ntr(SYNTH_IDCODE, SECURE_AREA_OFFSET as u32);
    ...
    fn arm9_offset_past_window_has_no_secure_area() {
        let rom = synth_ntr(SYNTH_IDCODE, SECURE_AREA_END as u32);
        assert!(matches!(state(&rom), Err(NtrError::NoSecureArea)));
    }
    ...
    fn garbage_secure_area_is_corrupt() {
        ...
        assert!(matches!(state(&rom), Err(NtrError::SecureAreaCorrupt)));
    }
    ...
    async fn encrypting_an_encrypted_rom_is_rejected() {
        ...
        .await
        .expect_err("rejects");
        assert!(matches!(err, NtrError::AlreadyEncrypted));
    }
