...
use crate::nintendo::ntr::info::{NtrSecureAreaState, crc16};
use crate::nintendo::ntr::{DECRYPTED_MARKER, HEADER_SIZE, SECURE_AREA_END, SECURE_AREA_OFFSET};

/// Game code baked into [`synth_ntr`].
pub const SYNTH_IDCODE: [u8; 4] = *b"ARCE";

/// Game title baked into [`make_ntr_info_fixture`], both the header field
/// and the banner's English title.
pub const SYNTH_TITLE: &str = "Test Game";

/// Banner block offset baked into [`make_ntr_info_fixture`].
const SYNTH_BANNER_OFFSET: u32 = 0x10000;

/// Total size of the images [`make_ntr_info_fixture`] and
/// [`make_ntr_info_fixture_no_banner`] build.
...
pub fn make_ntr_info_fixture(secure_state: NtrSecureAreaState) -> Vec<u8> {
    ...
    if secure_state != NtrSecureAreaState::NotPresent {
        let marker: [u8; 8] = if secure_state == NtrSecureAreaState::Decrypted {
            *DECRYPTED_MARKER
        } else {
            [0xAA; 8]
        };
...
pub fn make_ntr_info_fixture_no_banner() -> Vec<u8> {
    let header = build_info_header(NtrSecureAreaState::Decrypted, 0);
    let mut rom = vec![0u8; SYNTH_FILE_SIZE];
    rom[..HEADER_SIZE].copy_from_slice(&header);
...
fn build_info_header(secure_state: NtrSecureAreaState, banner_offset: u32) -> [u8; HEADER_SIZE] {
    ...
    let arm9_rom_offset: u32 = match secure_state {
        NtrSecureAreaState::NotPresent => SECURE_AREA_END as u32,
...
pub fn synth_ntr(idcode: [u8; 4], arm9_rom_offset: u32) -> Vec<u8> {
