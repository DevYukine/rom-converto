...
impl CompressionOffsetRecord {
    ...
    pub fn from_bytes(bytes: &[u8; OFFSET_RECORD_SIZE]) -> Self {
        ...
        for (i, s) in sizes.iter_mut().enumerate() {
            *s = u16_be(bytes, 8 + i * 2);
        }
        Self {
            base_offset: u64_be(bytes, 0),
            sizes,
        }
    }
}
...
impl FileDirectoryEntry {
    ...
    pub fn from_bytes(bytes: &[u8; FILE_DIRECTORY_ENTRY_SIZE]) -> Self {
        Self {
            name_offset_and_type_flag: u32_be(bytes, 0),
            word1: u32_be(bytes, 4),
            word2: u32_be(bytes, 8),
            word3: u32_be(bytes, 12),
        }
    }
}
...
mod tests {
    ...
    fn footer_fields_land_at_spec_offsets() {
        ...
        assert_eq!(u64_be(&bytes, 16), 0x1238);
        assert_eq!(u64_be(&bytes, 24), 40);
        assert_eq!(&bytes[96..128], &[0xABu8; 32]);
        assert_eq!(u64_be(&bytes, 128), 0x128C + FOOTER_SIZE as u64);
        assert_eq!(u32_be(&bytes, 136), FOOTER_VERSION);
        assert_eq!(u32_be(&bytes, 140), FOOTER_MAGIC);
    }
    ...
    fn offset_record_round_trips_through_bytes() {
        ...
        assert_eq!(u64_be(&bytes, 0), 0xDEAD_BEEF);
        assert_eq!(u16_be(&bytes, 38), 0xFFFF);
