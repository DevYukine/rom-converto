<script setup lang="ts">
import { storeToRefs } from "pinia";
import { useNxInfoStore } from "~/stores/nx-info";

const store = useNxInfoStore();
const { input, keys, info, rawJson, error, loading } = storeToRefs(store);

const NX_FILTERS = [
  { name: "Switch container", extensions: ["nsp", "nsz", "xci", "xcz"] },
];
</script>

<template>
  <div>
    <PageHeader
      title="Switch info"
      description="Inspect an NSP, NSZ, XCI, or XCZ. With prod.keys you get the full title id, version, NACP names, age ratings, and the JPEG icon. Without keys the command still surfaces container layout, ticket presence, and the file listing."
      :loading="loading"
      :has-result="!!info"
      :has-error="!!error"
    />

    <OperationCard>
      <div class="space-y-5">
        <FileDropZone
          v-model="input"
          label="Container"
          :primary="true"
          :filters="NX_FILTERS"
        />

        <FileDropZone
          v-model="keys"
          label="prod.keys (optional, falls back to ~/.switch/prod.keys)"
          :filters="[{ name: 'prod.keys', extensions: ['keys', 'txt'] }]"
        />

        <RunButton
          :loading="loading"
          :disabled="!input"
          @click="store.execute"
        >
          Read info
        </RunButton>

        <RomInfoCard v-if="info" :info="info" />
      </div>
    </OperationCard>

    <div class="mt-4">
      <OutputLog :error="error" />
    </div>

    <details v-if="rawJson" class="mt-4">
      <summary class="cursor-pointer text-sm text-zinc-500">Raw JSON payload</summary>
      <pre class="mt-2 overflow-auto rounded bg-black/40 p-3 text-xs">{{ rawJson }}</pre>
    </details>
  </div>
</template>
