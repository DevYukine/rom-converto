...
<template>
  <div class="min-w-0 space-y-1.5">
    ...
    <div
      v-else
      ref="dropZoneRef"
      class="drop-zone flex w-full min-w-0 flex-col items-center justify-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800/30 px-4 py-6 transition xl:py-8"
    >
      <svg aria-hidden="true" class="h-8 w-8 text-sky-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
      </svg>
      <div class="flex w-full min-w-0 justify-center text-sm font-medium text-zinc-200" :title="basename(modelValue)">
        <span class="min-w-0 shrink truncate">{{ splitFilenameForDisplay(modelValue)[0] }}</span>
        <span class="shrink-0">{{ splitFilenameForDisplay(modelValue)[1] }}</span>
      </div>
